...
type Chainable<S extends StringObject = {}> = {
  option: <K extends string, V extends any>(key: K extends keyof S ? never : K, value: V) => Chainable<Omit<S, K> & Record<K, V>>
  get(): S
}

declare const b: Chainable

const br = b.option('foo', 123).get()
